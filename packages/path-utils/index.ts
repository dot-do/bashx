 ...
 * @example
 * ```typescript
 * import { toUnixPath, expandPath, validatePathSecurity } from '@dotdo/path-utils'
 *
...
export function joinPath(...segments: string[]): string {
  ...
  if (segments.length === 1) return normalizePath(segments[0])

  let result = segments[0]
  for (let i = 1; i < segments.length; i++) {
    result = resolvePath(segments[i], result)
  }
  return result
}
...
export function dirname(path: string): string {
  ...
  const normalized = normalizePath(toUnixPath(path))
...
/**
 * Gets the base name of a path (the last segment).
 *
 ...
 * extname('/foo/bar/.hidden') // ''
 * extname('/foo/bar/noext')   // ''
 * ```
 */
 ...
 * @example
 * ```typescript
 * relativePath('/foo/bar', '/foo/baz')     // '../baz'
 * relativePath('/foo/bar', '/foo/bar/baz') // 'baz'
 * ```
 */
export function relativePath(from: string, to: string): string {
  const fromNorm = normalizePath(toUnixPath(from))
  const toNorm = normalizePath(toUnixPath(to))
